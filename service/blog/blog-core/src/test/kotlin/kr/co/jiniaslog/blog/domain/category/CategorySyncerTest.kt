...
class CategorySyncerTest : SimpleUnitTestContext() {
    ...
    fun `기존 카테고리가 존재하고 새로운 카테고리가 추가되면 동기화가 이루어진다`() {
        ...
        val newChild3Parent2 = SimpleCategoryVo(
            categoryId = null,
            categoryName = CategoryTitle("child3"),
            sortingPoint = 0,
            depth = 1
        )
        parent2Vo = parent2Vo.copy(
    ...
    fun `기존 카테고리가 존재하고 새로운 카테고리가 추가되며 무작위로 정렬되어 있어도 동기화는 이루어진다`() {
        ...
        val parent3Vo = SimpleCategoryVo(
            categoryId = null,
            categoryName = CategoryTitle("child3"),
            sortingPoint = 2,
            depth = 0
    ...
    private fun Category.toVo(): SimpleCategoryVo = SimpleCategoryVo(
        this.id,
        this.categoryTitle,
