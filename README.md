...
# 프로젝트 목표
이번 리부트, V2의 핵심 목표이자 키워드는 단순히 리뉴얼, 리메이크로 블로그를 다시여는것이 아니라, 그동안 학습한것들을 총동원하여 **오버아키텍처링으로** 때려넣고, 이를 지속가능한 방법론으로 발전시켜가는 구조를 실험해보는것입니다.

단순히 블로그 운영을 다시하거나, 동일 기능을 개발하는것은 아무런 기술적 챌린지도 되지않는다고 생각하였고, 차라리 공부한것들을 적용하고 실험하여 최종적으로는 개인 기술 자산이자 런타임 기술 포트폴리오로 확장하려는 욕심으로 다시 시작하게되었습니다.
...
또한 어떠한 환경에서도 안전하게 통합테스트가 수행될 수 있돌고 테스트컨테이너를 사용하였으며, 회귀 오류와 리팩토링 내성을 고려하여 목 사용을  최소화하는 테스트 코드 작성 방식을 택했습니다.

kotest assertion을 채택하거나 다양한 junit상의 기법들을 채택하여 테스트의 명료함과 가독성을 높이기위해 노력했습니다.
